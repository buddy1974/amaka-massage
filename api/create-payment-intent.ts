...
import { db, bookings, servicePrices, services } from './_lib'
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (rows.length === 0) return res.status(400).json({ error: 'Booking not found' })
    ...
    return res.status(200).json({ client_secret: paymentIntent.client_secret, amount: priceEur })
  } catch (err) {
