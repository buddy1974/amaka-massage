...
import { db, blockedSlots } from '../_lib'
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    ...
    if (date) {
      return res.json(await db.select().from(blockedSlots).where(eq(blockedSlots.blockDate, date)))
    }
    if (from && to) {
      return res.json(await db.select().from(blockedSlots)
