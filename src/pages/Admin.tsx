...
import {
  ...
  ChevronLeft, ChevronRight, Plus, Check, X, Phone, Clock,
  Flower2,
} from 'lucide-react'
...
const Admin = () => {
  ...
  // ── bulk-add default slots for a day ──
  const addDefaultSlots = async () => {
    ...
    try {
      await fetch('/api/admin/slots', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...authHeader() },
        body: JSON.stringify({ date: selectedDate, times: DEFAULT_TIMES }),
      })
      toast.success(`Slots added for ${selectedDate}`)
      await fetchSlots()
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <Button variant="outline" size="sm" onClick={fetchBookings} disabled={loadingBookings}>
                <RefreshCw className={`h-4 w-4 mr-1 ${loadingBookings ? 'animate-spin' : ''}`} />
                Refresh
              </Button>
            </div>
            ...
            {bookings.length === 0 && !loadingBookings && (
              <div className="text-center py-20 text-muted-foreground">
                <Flower2 className="h-10 w-10 mx-auto mb-3 text-primary/40" />
                No bookings yet.
              </div>
            )}
        ...
        {tab === 'slots' && (
          <div>
            <div className="flex items-center gap-4 mb-6 flex-wrap">
              ...
              <Button onClick={addDefaultSlots} disabled={addingSlots} className="gradient-purple text-primary-foreground hover:opacity-90">
                <Plus className="h-4 w-4 mr-1" /> {addingSlots ? 'Adding…' : 'Add Default Slots'}
              </Button>
            ...
            {slots.length === 0 && !loadingSlots && (
              <div className="text-center py-16 text-muted-foreground">
                ...
                No slots for this date. Click "Add Default Slots" to add 10:00–18:30.
              </div>
            )}
              ...
              {slots
                ...
                .map((slot) => {
                  ...
                  return (
                    <button
                      key={slot.id}
                      onClick={() => !isLocked && toggleSlot(slot)}
                      disabled={!!isLocked}
                    ...
                    >
                      ...
                      <div className="text-xs mt-0.5 opacity-70">
                        {isLocked ? '🔒 locked' : slot.isAvailable ? '✓ open' : '✗ closed'}
                      </div>
                    </button>
                  )
                })}
            ...
            <p className="text-xs text-muted-foreground mt-4">
              Click any slot to toggle open / closed. Locked slots are currently in the booking flow — they auto-release after 10 minutes.
            </p>
          </div>
        )}
            ...
            <div className="bg-card rounded-2xl shadow-card overflow-hidden">
              {Array.from(
                bookings.reduce((acc, b) => {
                  ...
                  const cur = acc.get(key) ?? { count: 0, revenue: 0 }
                  acc.set(key, { count: cur.count + 1, revenue: cur.revenue + parseFloat(b.priceEur) })
                  return acc
